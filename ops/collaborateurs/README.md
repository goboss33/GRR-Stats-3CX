...
**On peut revenir en arrière.** L'assistant est un parcours de sections, pas un tunnel : **Échap** dans une liste, **q** dans un champ, ramènent à la question précédente — les sections qui ne posent rien (lectures, calculs) sont sautées. Au récapitulatif, répondre **Non** n'arrête plus le script : une liste « Que voulez-vous modifier ? » montre chaque réponse donnée, on en choisit une, et le parcours reprend de là vers l'avant. Chaque question repose alors sa réponse précédente comme défaut — Entrée pour la garder — et tout ce qui en dépend est recalculé (identifiant, adresse, lecture du central). Quitter reste possible depuis cette liste, ou par Ctrl+C.
...
**Sous quel compte l'annuaire est écrit.** Sur le contrôleur de domaine lui-même, le script ne demande aucun identifiant : les cmdlets AD tournent sous le **compte de la session Windows**, celui qui a ouvert la console. C'est donc cette session qui doit avoir le droit de créer, modifier et déplacer des utilisateurs dans les OU visées. Le récapitulatif l'annonce sur la ligne « Écriture AD » avant la confirmation. **Et la console doit être élevée** (« Exécuter en tant qu'administrateur ») : sans cela, l'UAC ampute le jeton de la session des groupes d'administration, et sur le contrôleur l'annuaire répond « Accès refusé » aux créations et modifications, même pour un administrateur du domaine — les lectures, elles, passent. Le 10.09.2026, deux vraies entrées lancées depuis une console non élevée se sont arrêtées ainsi à la création du compte, avant toute autre écriture. Le script le signale dès la connexion au domaine et sur la ligne « Écriture AD » du récapitulatif. Depuis une machine hors du domaine, le script demande les identifiants une fois par domaine, en proposant le compte indiqué dans `config.json` → `operateurs`.
...
**Sortie** : retrouve le poste par l'e-mail, puis demande s'il faut le **supprimer**. Non : il reste, désactivé, e-mail vidé, renommé **« Libre <file> »**, réservé à son équipe. Oui : il est supprimé du central. Ses **numéros directs** (SDA) sont dans les deux cas renommés **« ex Prénom Nom (date du jour) »**. S'il est supprimé, ils sont en plus redirigés vers **sa file d'attente**, aux heures ouvrables comme en dehors ; s'il reste, leur destination ne bouge pas. La file sert aussi à nommer le poste libéré. Elle est déduite quand le poste n'est que dans une seule ; s'il est dans plusieurs, ou dans aucune, le script demande laquelle, dans la liste de toutes les files, filtrable par numéro, par nom de file ou par nom de collègue. Le poste est retiré de ses files avant d'être libéré ou supprimé. Si aucun poste ne porte l'adresse du compte, le script propose de le désigner à la main dans la liste des postes du central ; `-Poste3CX 128` le désigne d'emblée, ce qui sert à tester la sortie sur un poste d'essai avec `-Simulation -Reel3CX` : le compte choisi à l'écran n'est pas touché, seul le poste désigné l'est.
