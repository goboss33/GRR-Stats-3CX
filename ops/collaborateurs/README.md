...
**Sous quel compte l'annuaire est écrit.** Sur le contrôleur de domaine lui-même, le script ne demande aucun identifiant : les cmdlets AD tournent sous le **compte de la session Windows**, celui qui a ouvert la console. C'est donc cette session qui doit avoir le droit de créer, modifier et déplacer des utilisateurs dans les OU visées. Le récapitulatif l'annonce sur la ligne « Écriture AD » avant la confirmation. Le 10.09.2026, une vraie entrée lancée depuis une session sans ce droit s'est arrêtée sur « Accès refusé » à la création du compte, avant toute autre écriture. Depuis une machine hors du domaine, le script demande les identifiants une fois par domaine, en proposant le compte indiqué dans `config.json` → `operateurs`.
