...
<#
    ...
    Commencez TOUJOURS par -Simulation sur un vrai compte : tout est décrit,
    rien n'est écrit — chaque écriture passe par Invoke-Ecriture, y compris
    Exchange Online.
...
if ($Job) {
    ...
    Show-Section '2 · Messagerie'

    # --- Redirection : vers un utilisateur ou une liste, choisi dans une recherche, plus d'identifiant tapé.
    $optionsRedirection = @(
        [pscustomobject]@{ Code = 'activer';    Texte = "Rediriger les mails de $($dossier.Upn) vers un collaborateur ou une liste" },
    ...
    } elseif ($r.Code -eq 'desactiver') { $dossier.Redirection = 'desactiver' }

    # --- Réponse automatique : un modèle de config.json, ou un texte tapé / collé ; aperçu avant de retenir.
    ...
    do {
        ...
        switch ($a.Code) {
            ...
            'desactiver' { $dossier.ReponseAuto = 'desactiver' }
...
if ($pbx) {
    ...
    if ($lecture) { $dossier.Poste3CX = $lecture.Poste; $dossier.Files3CX = @($lecture.Files); $dossier.Sda3CX = @($lecture.Sda) }
}
...
try {
    ...
    Invoke-Etape -Nom 'Groupes AD retirés' -Categorie Groupes -Action {
        ...
        Add-Journal -Message "Groupes $(if (Test-Simulation) { 'à retirer' } else { 'retirés' }) ($($retires.Count)) : $($retires -join '; ')" -Categorie Groupes -Niveau Succes
    ...
    Invoke-Etape -Nom 'Réponse automatique' -Categorie Exchange -Ignorer:(-not $m365 -or $dossier.ReponseAuto -eq 'aucune') -Action {
        if ($dossier.ReponseAuto -eq 'activer') {
            ...
            Invoke-Ecriture -Categorie Exchange -Description "Set-MailboxAutoReplyConfiguration $upn -AutoReplyState Enabled — « $($dossier.ReponseAutoTexte -replace "`r?`n", ' / ') »" -Action {
                Set-MailboxAutoReplyConfiguration -Identity $upn -AutoReplyState Enabled -InternalMessage $htmlAuto -ExternalMessage $htmlAuto
                Add-Journal -Message "Réponse automatique activée : $($dossier.ReponseAutoTexte -replace "`r?`n", ' / ')" -Categorie Exchange -Niveau Succes
            } | Out-Null
        } else {
