...
const decisions: Decision[] = [
    ...
    },
    {
        ...
        summary: "Sans taux d'activité RH, la question « ce collaborateur est-il là, prêt à recevoir les appels ? » se mesure au PBX : toutes les minutes, l'état de chaque poste est relevé dans le XAPI (profil disponible / personnalisé / absent / ne pas déranger, connexion aux files — l'icône Q —, téléphone enregistré) et seuls les changements sont conservés. Chaque jour est ventilé sur les heures de bureau du département 3CX (pauses et fériés déduits ; horaires de repli 08:00–17:30 pour un département sans horaires déclarés). Les PROFILS PERSONNALISÉS du 3CX (« Custom 1 », bleus dans le client) ont leur état à eux : le PBX ne dit pas ce qu'ils font, et ceux d'ici veulent dire « joignable autrement » — le « Custom 1 » d'une gérante s'appelle « En séance · sonne 5 s », et elle décroche 16 des 131 sonneries de file qu'elle reçoit en neuf jours. Ils ne comptent PAS dans la disponibilité, qui reste stricte (arbitrage du 9 septembre 2026), mais ne sont plus rangés parmi les absences. L'annuaire des collaborateurs montre l'état au dernier relevé (pastille sur l'avatar, avec le nom d'usage du profil), la part de temps de chaque état sur 30 jours, et — dans une colonne à part, triable — la part de temps connecté aux files.",
    ...
    },
    {
        ...
        date: "8 septembre 2026",
        summary: "L'écran détail d'une équipe se termine par deux cartes côte à côte. À gauche, le classement des équipes qui lui envoient des appels : les appels reçus parce qu'une autre équipe ne les a pas pris (débordement) ou les a transmis. L'équipe d'origine est la file sollicitée immédiatement avant la nôtre dans le même appel — pas toutes les files traversées. À droite, où partent ses appels transférés (décrochés ici, servis ailleurs) et débordés (partis sans décroché), directs compris : une ligne par équipe de destination, avec les visages des personnes qui ont pris ces appels — toutes, pas un top 10. La destination est le PREMIER saut après nous, dans l'ordre du temps : la ligne directe visée par un transfert — que la personne réponde ou non —, sinon la file sollicitée. Le nombre porté par un visage compte les appels qui lui sont PARTIS, sans distinguer ceux qu'il a pris : c'est la question à laquelle la carte répond. Une personne est rattachée à son équipe principale (règle ci-dessous) ; les numéros externes et les personnes membres d'aucune équipe font chacun leur regroupement. Le chemin emprunté entre les deux (script, groupe d'appel, renvoi de poste, ou plus rien depuis les horaires natifs) est ignoré. La population est celle des vignettes (queue_calls) : le total du graphique est un sous-ensemble exact de « Appels reçus » — une ligne par appel, un appel repassé plusieurs fois dans la file gardant la provenance du passage retenu par la règle multiPassage (arrivé directement, parti puis revenu, il n'est pas « venu d'ailleurs ») — et chaque barre s'ouvre sur les journaux correspondants pour qui en a le droit.",
        justification: "Remonter la chaîne des parents (originating_cdr_id) s'arrête au groupe d'appel ou au script intercalé et dépend de la configuration 3CX du moment ; regarder le passage en file précédent du même appel est le miroir exact de la détection du débordement, insensible à l'assainissement du PBX (vérifié sur l'ancien routage d'août 2026 et sur le routage direct du 7 septembre). Mesuré en août 2026 : 29 % des appels de la réception de Genève venaient de 39 équipes, 39 % de ceux de la réception de Pully de 13 équipes ; les transferts après décroché chez l'équipe d'origine sont marginaux (2 sur 2 237), il s'agit de débordement quasi pur. Côté départs, une réception transfère surtout vers des lignes directes (80 % des 1 070 départs de la 958, vers 229 personnes — un top 10 ne couvrirait que 25 % du volume), une gérance déborde vers sa réception : d'où des lignes par équipe qui concentrent, et des visages qui montrent tout le monde. Règle de l'équipe principale : les appartenances du journal de composition XAPI (configuration 3CX officielle, datée) quand il couvre la période, sinon les files qui ont sollicité la personne ; parmi elles, celle qui lui a distribué le plus d'appels sur la période, à égalité la plus récente. Mesuré : ~1 % des départs concernent une personne dont la première file pèse moins de 80 % — le choix est rare et il est annoncé (« aussi dans … ») plutôt que caché.",
        impact: "Nouveau fait from_queue dans queue_passage_facts (LATERAL à rebours, ~0,06 s par mois sur la plus grosse file), porté par call_queue_outcomes ; l'API queue expose inboundSources, ainsi que les sorties brutes outboundExits (première destination par appel parti) et personTeams (sollicitations des personnes jointes en direct). Le service compose les lignes par équipe (journal XAPI via resolveJournalMemberships, annuaire, photos et titres des collaborateurs) et applique la règle « qui voit quoi » (outOfScopeFinalStatus) aux deux cartes : équipes nommées, ou fondues en un regroupement « Équipes hors périmètre » — les visages hors périmètre restent affichés (arbitrage du 8 septembre 2026). Un visage ne s'affiche sous une équipe que si la personne en est MEMBRE, c'est-à-dire sonnée par cette file sur la période : quelqu'un à qui la réception passe un appel, ou qui récupère son propre appel en attente, a bien pris l'appel mais n'est pas de la maison (cas relevé le 8 septembre 2026 : Lulzim Adzami, Gérance PU-C03, apparaissait parmi les visages de la Réception Pully). Une ligne peut donc porter plus d'appels que la somme de ses visages : ceux qui ont débordé vers une file sans que personne y décroche n'ont personne à nommer. L'écart n'est pas signalé à l'écran (arbitrage du 8 septembre 2026) — il ne concerne que les débordements, les transferts étant tous nommés. Les visages mènent aux journaux (« Traité par ») ; les lignes d'équipe pas encore, faute de filtre exact « parti vers cette équipe, ligne directe comprise ». Le champ overflowDestinations de l'API, qui mélange amont et aval, est conservé tel quel pour ses consommateurs et déprécié.",
    },
    {
        id: "1.17",
        category: "Statistiques Queue",
