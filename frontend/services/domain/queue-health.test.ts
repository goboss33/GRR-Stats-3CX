...
describe("assessQueueHealth", () => {
    it("est OK avec des collaborateurs actifs et des appels récents", () => {
    ...
    it("est critique sans aucun collaborateur rattaché", () => {
        const h = assessQueueHealth({ agents: [], lastCallAt: daysAgo(1), status: "ACTIVE" }, NOW);
        expect(h.level).toBe("critical");
        expect(h.reasons[0]).toContain("Aucun collaborateur rattaché");
    });

    it("est critique quand plus aucun collaborateur n'est actif", () => {
        const h = assessQueueHealth(
    ...
    it("avertit quand certains collaborateurs sont inactifs de longue date", () => {
        const h = assessQueueHealth(
