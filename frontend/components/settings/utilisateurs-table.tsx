...
type Colonne = "nom" | "poste" | "email" | "equipes" | "etat" | "depuis" | "compte" | "presence" | "presenceFile" | "perimetre" | "activite" | "cree";
...
const COLONNES: Record<Colonne, DefinitionColonne<CollaborateurRow>> = {
    ...
    activite: { type: "date", valeur: (c) => c.compte?.lastSeenAt ?? c.compte?.lastLoginAt ?? null },
...
const TRI_DE: Partial<Record<CleColonne, Colonne>> = {
    ...
    email: "email", m365: "etat", files: "presenceFile", perimetre: "perimetre", activite: "activite", cree: "cree", depuis: "depuis",
};
                                    ...
                                    >
                                        {colonnesAffichees.map((col) => {
                                            switch (col.cle) {
                                                ...
                                                case "activite": {
                                                    const quand = c.compte?.lastSeenAt ?? c.compte?.lastLoginAt ?? null;
                                                    return <td key={col.cle} className="px-4 py-2 text-xs text-slate-500">{quand ? new Date(quand).toLocaleString("fr-CH", { dateStyle: "short", timeStyle: "short" }) : <span className="text-slate-400">—</span>}</td>;
                                                }
...
/** Le compte de l'application : sa nature, son mode de connexion, sa dernière venue — ou ce qui l'attend. */
