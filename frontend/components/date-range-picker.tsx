 ...
 * 
 * A reusable date range picker with:
 * - Quick presets (Today, 7 days, 30 days, 90 days)
 * - Two-click selection (start date, then end date)
 * - Live preview while hovering
 * - Two-month calendar view
 * - French locale support
 * 
 * Based on the ColumnFilterDateRange implementation from the logs page.
 * This component is designed to be reused across the application.
 * 
 * @example
 * <DateRangePicker
...
export interface DateRangePickerProps {
    ...
    className?: string;
    /** Size variant - 'default' for normal pages, 'compact' for column filters */
...
export function DateRangePicker({
    ...
    // Selection state (null when not actively selecting)
    ...
    // Reset selection state when opening/closing
    ...
    // Handle day click (two-click selection)
    ...
    const formatDisplay = () => {
        if (displayFormat === "long") {
            return `${format(dateRange.startDate, "dd MMM yyyy", { locale: fr })} - ${format(dateRange.endDate, "dd MMM yyyy", { locale: fr })}`;
        }
        return `${format(dateRange.startDate, "dd/MM", { locale: fr })} - ${format(dateRange.endDate, "dd/MM", { locale: fr })}`;
    };

    // Help text shown during selection
    const helpText = () => {
        if (!startDate) {
    ...
    // Check if a day is in the preview range (during hover)
    ...
    // Check if a day is in the applied (committed) range
