...
type SortField = "name" | "attempts" | "answered" | "transferred" | "answerRate" | "availabilityRate" | "avgHandlingTimeSeconds" | "totalHandlingTimeSeconds";
...
export function AgentPerformanceTable({ agents }: AgentPerformanceTableProps) {
    ...
    const totals = agents.reduce(
        ...
        { attempts: 0, answered: 0, transferred: 0, totalHandlingTimeSeconds: 0 }
    );
    const totalAnswerRate = totals.attempts > 0 ? Math.round((totals.answered / totals.attempts) * 100) : 0;
                    ...
                    <table className="w-full">
                        <thead className="bg-slate-50 border-y">
                            <tr>
                                ...
                                <SortHeader field="attempts" label="Tentatives" />
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {sortedAgents.map((agent, index) => (
                                <tr key={`${agent.extension}-${agent.name}-${index}`} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 font-semibold text-slate-900">{agent.answered}</td>
                                    <td className="px-4 py-3">
                                        {agent.transferred > 0 ? (
                        ...
                        <tfoot className="bg-slate-100 border-t-2 border-slate-300">
                            <tr className="font-semibold">
                                ...
                                <td className="px-4 py-3 text-slate-800">{totals.attempts}</td>
                                <td className="px-4 py-3 text-slate-900">{totals.answered}</td>
                                <td className="px-4 py-3 text-slate-800">{totals.transferred}</td>
                                <td className="px-4 py-3">
